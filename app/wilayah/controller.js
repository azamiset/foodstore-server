...
async function getKabupatem(req, res, next) {
...
async function getDesa(req, res, next) {
  ...
  try {
    ...
    return res.json(data.filter((desa) => desa.kode_kecamtan === kode_induk));
...
module.exports = { getProvinsi, getKabupatem, getKecamatan, getDesa };